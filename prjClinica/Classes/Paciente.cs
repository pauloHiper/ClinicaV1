...
namespace prjClinica.Classes
{
    ...
    public class Paciente : TabelaBase, IComparable<Paciente>
    {
        ...
        public static List<Paciente> buscaLike(String nome, Conexao con)
        {
            ...
            String sql = String.Concat("SELECT id", tabela, " FROM ", nomeTabela, " where nome like @1 and stAtivo = 1 order by nome");
        ...
        public static Paciente busca(int id, Conexao con)
        {
            try
            {
                String sql = String.Concat("SELECT ", campos, " FROM ", nomeTabela, " WHERE ", nomeId, "=", id, " and stAtivo=1");
        ...
        public static List<Paciente> listaPacientes(Conexao con, char order)
        {
            try
            {
                List<Paciente> lista = new List<Paciente>();
                ...
                if (order == 'n')
                    ...
                    return lista.OrderBy(value => value.idPaciente).ToList();
                else
                    return lista;
            }
            catch (Exception)
